function light_evoked_grouped = check_light_response_zscore(spiketime,stim_grouped,sniff_onset, varargin)
% based on check_light_response_ttest.m
...
for j = 1:length(stim_grouped)
    ...
    zscorethreshold = getOr(option, 'zscorethreshold', 10);
    ...
    if ~isnan(ttest(fr_base,fr_base0))
        % insead of ttest, we use z-score
        zscored_response = (fr_base-fr_base0)./std(fr_base0);
        if (sum(zscored_response>zscorethreshold))>pulse_num
...
%         if(ttest(fr_base,fr_base0)) ==1 % light response different from the baseline
            if sum(fr_base)>sum(fr_base0) % if firing rate increased
                ...
                if plotevoked
                    ...
                    subplot 212
                    plot(bins_base, zscored_response)
%                     plot(bins_base,fr_base)
%                     hold on
